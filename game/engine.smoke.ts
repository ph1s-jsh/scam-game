...
function messageBeat(state: GameState, threadId: string, marker: string) {
  ...
  return next.pendingNpcTurns.length ? replyPending(next, 'Ừ, mình đọc rồi.') : next;
}
...
function reachAnTask(runId = 'an-task') {
  ...
  state = replyPending(state, 'Em đọc phần mô tả trước nhé.');
...
// Scheduling filters spam only; the model interprets ordinary conversation.
{
  const base = start('hanh', 'reply-policy');

  const acknowledged = send(base, 'hanh-family', 'ok', 'ack-turn');
  assert.equal(acknowledged.pendingNpcTurns[0]?.responseKind, 'ai');
  assert.equal(
  ...
  assert.equal(thanked.pendingNpcTurns[0]?.responseKind, 'ai');
  ...
  assert.ok(forwardedLink.pendingNpcTurns[0]?.delayMs >= 3_500);
  ...
  assert.equal(boundary.pendingNpcTurns[0]?.responseKind, 'ai');
  assert.equal(boundary.pendingNpcTurns[0]?.localReply, undefined);
  boundary = replyPending(boundary, 'Bà đang bực chuyện gì ạ?');
  const repeatedAbuse = send(boundary, 'hanh-an-real', 'Cút đi, đồ ngu', 'boundary-two');
  assert.equal(repeatedAbuse.pendingNpcTurns[0]?.responseKind, 'ai');
  ...
  assert.equal(duplicateAgain.pendingNpcTurns.length, 1);
...
// Every conversation turn is bound to a concrete, isolated AI character.
{
  ...
  assert.ok(
    state.pendingNpcTurns[0]?.directorPlan?.factCatalog.some(
      (fact) => fact.id === 'role:family.an',
    ),
  );
...
// The director selects a group speaker deterministically and keeps a natural turn owner.
{
  ...
  assert.ok(!groupKnowledge.factCatalog.some((fact) => /Bảo đang ở lớp đến 20 giờ/.test(fact.text)));
  assert.equal(
    validateNpcDirectorReply({
      plan: groupKnowledge,
  ...
  assert.ok(
    contextualQuestion.pendingNpcTurns[0]?.directorPlan?.factCatalog.some(
      (fact) => fact.id.startsWith('dialogue:'),
    ),
  );
  ...
  assert.equal(paymentRefusal.pendingNpcTurns[0]?.directorPlan?.move, 'answer');
  ...
  assert.equal(noMoneyPlan?.move, 'answer');
  ...
  assert.deepEqual(noMoneyFailure.messages, noMoney.messages);
  ...
  assert.equal(fraudNoMoney.pendingNpcTurns[0]?.directorPlan?.move, 'answer');
  assert.equal(
    fraudNoMoney.pendingNpcTurns[0]?.directorPlan?.interactionMode,
    'coercive',
  );
  assert.deepEqual(failPending(fraudNoMoney).messages, fraudNoMoney.messages);
  ...
  assert.equal(bareQuestion.pendingNpcTurns[0]?.directorPlan?.move, 'answer');
  ...
  assert.equal(
    validateNpcDirectorReply({
      plan: privateRecallPlan,
      reply:
        'Dạ, con không đọc được cuộc trò chuyện riêng giữa bà với Bảo đâu ạ.',
      move: 'answer',
      factIdsUsed: privateRecallPlan.requiredFactIds,
    }),
    true,
  ...
  assert.deepEqual(privateRecallFailure.messages, privateRecall.messages);
}

// Failed generation is not dialogue or a completed action.
{
  const before = send(start('hanh', 'failure'), 'hanh-an-real', 'Con khỏe không?', 'failed-1');
  const failed = failPending(before);
  assert.deepEqual(failed.messages, before.messages);
  assert.deepEqual(failed.transactions, before.transactions);
  assert.deepEqual(failed.requestStatus, before.requestStatus);
  assert.equal(failed.failedNpcTurns?.length, 1);
  const retry = gameReducer(failed, {type: 'RETRY_NPC_TURN', failedTurnId: 'failed-1', turnId: 'retry-1'});
  assert.deepEqual(retry.messages, before.messages);
  assert.equal(retry.pendingNpcTurns[0]?.id, 'retry-1');
  assert.equal(retry.failedNpcTurns?.length, 0);
  assert.equal(gameReducer(retry, replyAction(before, 'Câu trả lời cũ.')), retry);
  const repaired = replyPending(retry, 'Dạ con khỏe bà ạ.');
  assert.equal(repaired.pendingNpcTurns.length, 0);
  assert.equal(repaired.messages['hanh-an-real'].at(-1)?.text, 'Dạ con khỏe bà ạ.');
}
...
// A short contextual acceptance is answered, while scripted messages wait for it.
{
  ...
  assert.equal(state.tick, beforeAcceptance);
  ...
  state = messageBeat(state, 'an-family', 'after-short-acceptance');
  assert.ok(state.triggeredEventIds.includes('an-event-reward'));
  assert.equal(
...
// Legitimate cash and family arrangements resolve only after the NPC turn, without touching the player's bank balance.
{
  ...
  assert.equal(contactPending?.settlementOnReply, undefined);
  assert.equal(contactPending?.responseKind, 'ai');
  assert.ok(
    contactPending?.directorPlan?.sceneContracts?.some((contract) =>
      contract.forbiddenActs.includes('npc-funds-current-order'),
    ),
  );
  ...
  for (const impossible of [
    'Con sẽ chuyển ngay rồi mai tính.',
    'Con trả hộ bà và bà không cần hoàn lại.',
    'Con có đủ tiền.',
    'Con có thể gọi hộ nhà thuốc cho bà.',
  ]) {
    assert.equal(
      validateNpcDirectorReply({
        plan: contactPending.directorPlan!,
        reply: impossible,
        move: 'answer',
        factIdsUsed: [],
      }),
  ...
  const failedCash = failPending(cashOnDelivery);
  assert.equal(failedCash.requestStatus['hanh-pay-pharmacy'], 'pending');
  cashOnDelivery = replyPending(cashOnDelivery, configuredSettlementReply(cashOnDelivery));
  ...
  assert.equal(anCash.pendingNpcTurns[0]?.settlementOnReply, undefined);
  anCash = replyPending(anCash, 'Bà chưa có tiền sẵn để đóng, con kiểm tra hóa đơn rồi đóng giúp bà nhé.');
  assert.equal(anCash.requestStatus['an-pay-internet'], 'pending');
  ...
  assert.equal(
    choosesCashInstead.pendingNpcTurns[0]?.settlementOnReply,
    undefined,
  );
  ...
  assert.equal(familyAn.pendingNpcTurns[0]?.settlementOnReply, undefined);
...
// Restored sessions discard stale or malformed pending turns instead of locking chat.
{
  ...
  values.set(
    ...
    JSON.stringify({
      ...pendingState,
      requestStatus: {
        ...pendingState.requestStatus,
        'hanh-pay-pharmacy': 'arranged',
      },
      requestArrangementOptionIds: {
        'hanh-pay-pharmacy': 'hanh-an-pays-cash-back',
      },
      pendingNpcTurns: [
        {
          ...pendingState.pendingNpcTurns[0],
          settlementOnReply: {
            requestId: 'hanh-pay-pharmacy',
            optionId: 'hanh-an-pays-cash-back',
          },
          responseGuidance: 'obsolete promise',
        },
      ],
    }),
  );
  const migrated = loadGameState()!;
  assert.equal(migrated.requestStatus['hanh-pay-pharmacy'], 'pending');
  assert.equal(
    migrated.requestArrangementOptionIds['hanh-pay-pharmacy'],
    undefined,
  );
  assert.equal(migrated.pendingNpcTurns[0].settlementOnReply, undefined);
  assert.equal(migrated.pendingNpcTurns[0].responseGuidance, undefined);
  assert.deepEqual(migrated.messages, pendingState.messages);
  assert.deepEqual(migrated.transactions, pendingState.transactions);
...
// Each role has a safe route: ordinary mistakes may lower quality, but only fraud causes a loss.
{
  ...
  hanhCash = send(
    hanhCash,
    'hanh-an-real',
    'Bà sẽ trả đúng 186.000đ tiền mặt khi nhận thuốc nhé.',
    'safe-hanh-cash-turn',
  );
  ...
  if (hanhCash.pendingNpcTurns.length) {
    const pending = hanhCash.pendingNpcTurns[0];
    assert.fail(
      npcReplyRejectionReason({
        plan: pending.directorPlan!,
        reply: configuredSettlementReply(hanhCash),
        move: pending.directorPlan!.move,
        factIdsUsed: pending.directorPlan!.requiredFactIds,
      }) ?? 'cash turn not committed',
    );
  }
  hanhCash = act(
    hanhCash,
  ...
  assert.ok(
    hanhCashDebrief?.timeline.some((item) =>
      item.includes('tiền mặt khi nhận'),
    ),
  );
...
// Screenshot regressions: natural follow-ups, bounded capabilities and real actions.
{
  const scenario = getScenario('hanh')!;
  let conversation = start('hanh', 'free-dialogue');
  const turns = [
    [
      'con tự chuyển khoản cho người ta được không',
      'Dạ hôm nay con chưa có tiền. Bà trả giúp đơn này, mai con gửi lại bà nhé.',
    ],
    [
      'bà làm gì có tiền đâu con',
      'Dạ con cũng chưa có tiền để trả hộ. Bà chưa có tiền mặt hay đang khó thao tác trên điện thoại ạ?',
    ],
    [
      'Vậy còn cách nào khác không?',
      'Bà có thể chuyển khoản hoặc trả tiền mặt lúc nhận đúng túi thuốc ạ.',
    ],
    [
      'Người ta đem thuốc tới rồi con ơi',
      'Bà kiểm tra đúng túi tên Hạnh nhé. Bà muốn chuyển khoản hay trả tiền mặt ạ?',
    ],
  ];
  for (const [index, [question, answer]] of turns.entries()) {
    conversation = send(
      conversation,
      'hanh-an-real',
      question,
    ...
    const plan = conversation.pendingNpcTurns[0]?.directorPlan;
    assert.ok(plan);
    assert.equal(
      validateNpcDirectorReply({
        plan,
    ...
    conversation = gameReducer(conversation, {
      ...replyAction(conversation, answer, 'ai'),
      move: 'answer',
    });
    assert.equal(conversation.pendingNpcTurns.length, 0);
    assert.equal(conversation.requestStatus['hanh-pay-pharmacy'], 'pending');
  }
  const recall = send(
    conversation,
    'hanh-an-real',
    'Bảo vừa nhắn gì trong nhóm?',
    'shared-recall-speaker',
  );
  const recallPlan = recall.pendingNpcTurns[0]!.directorPlan!;
  assert.ok(recallPlan.factCatalog.some((fact) => fact.id === 'shared-dialogue:hm-family-1'));
  assert.ok(
    !recallPlan.factCatalog.some(
      (fact) => fact.id === 'shared-dialogue:hm-family-2',
    ),
  );

  let delivery = start('hanh', 'cancel-real');
  delivery = gameReducer(delivery, {
    type: 'OPEN_THREAD',
    threadId: 'hanh-an-real',
  });
  delivery = messageBeat(delivery, 'hanh-an-real', 'cancel-reveal');
  assert.ok(delivery.triggeredEventIds.includes('hanh-event-pharmacy'));
  for (const [index, phrase] of [
    'Cô không có tiền, hủy đơn giúp cô.',
    'Đúng rồi, hủy đơn giúp cô.',
    'Cô không muốn nhận nữa, hủy đơn đi.',
  ].entries()) {
    ...
    const plan = cancellation.pendingNpcTurns[0]!.directorPlan!;
    assert.equal(plan.settlementMethod, 'cancel-order');
    for (const wrong of [
      'Dạ cô vẫn thanh toán như thường nhé.',
      'Dạ cô cứ yên tâm nhé.',
      'Dạ cô đợi nhận thuốc rồi trả tiền giúp nhà thuốc nhé.',
    ])
      assert.equal(
        validateNpcDirectorReply({
          plan,
    ...
    assert.equal(
      npcReplyRejectionReason({
        plan,
        reply: configuredSettlementReply(cancellation),
        move: 'confirm',
        factIdsUsed: plan.requiredFactIds,
      }),
      null,
    ...
    const confirmed = gameReducer(
      cancellation,
      replyAction(cancellation, configuredSettlementReply(cancellation), 'ai'),
    );
    assert.equal(confirmed.requestStatus['hanh-pay-pharmacy'], 'cancelled');
    assert.equal(
      currentBalance(confirmed, scenario),
      currentBalance(delivery, scenario),
    );
    assert.strictEqual(
      gameReducer(confirmed, pay('hanh-pay-pharmacy')),
      confirmed,
    );
  }
  for (const [index, phrase] of [
    ...
    assert.equal(
      question.pendingNpcTurns[0]?.settlementOnReply,
      undefined,
      phrase,
    );
    assert.equal(question.requestStatus['hanh-pay-pharmacy'], 'pending');
  }

  const first = send(
    start('hanh', 'burst'),
    'hanh-an-real',
    'Con tự thanh toán giúp bà nhé',
    'burst-1',
  );
  const second = send(
    first,
    'hanh-an-real',
    'Ý bà là bà chưa có tiền',
    'burst-2',
  );
  assert.deepEqual(
    second.pendingNpcTurns.map((turn) => turn.id),
    ['burst-2'],
  );
  assert.strictEqual(
    gameReducer(second, replyAction(first, 'Câu trả lời cũ', 'ai')),
    second,
  );
  ...
  assert.equal(duplicate.pendingNpcTurns[0]?.id, 'burst-2');
  assert.equal(
    npcWorldRevision(duplicate, scenario, duplicate.pendingNpcTurns[0]),
    second.pendingNpcTurns[0]!.directorPlan!.baseRevision,
  );
  const number = send(
    start('hanh', 'number'),
    'hanh-an-real',
  ...
  assert.equal(number.pendingNpcTurns.length, 1);
}
